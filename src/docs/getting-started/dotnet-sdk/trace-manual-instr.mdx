...
The AWS Distro for OpenTelemetry .NET SDK contains extension library for AWS X-Ray and AWS SDK instrumentation. In this tutorial, we will introduce how to manually instrument your application step-by-step using AWS Distro for OpenTelemetry .NET SDK.
...
The OpenTelemetry SDK for .NET deals with concerns such as sampling, processing pipeline, and exporting telemetry to a particular backend.
...
Configure AWS X-Ray id generator, propagator and OTLP exporter globally in your application as follows. Make sure to call `AddXRayTraceId()` in the very **beginning** when creating `TracerProvider`
...
In .NET, you can use activity API to create custom spans to monitor the performance of internal activities that are not captured by instrumentation libraries. 
...
For more information about activity API, see the [OpenTelemetry .NET SDK's developer guide](https://github.com/open-telemetry/opentelemetry-dotnet/tree/main/src/OpenTelemetry.Api#instrumenting-a-libraryapplication-with-net-activity-api).
