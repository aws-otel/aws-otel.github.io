...
If you plan to call an AWS service or another application instrumented with AWS X-Ray SDK, you’ll need to configure the `AWSXRayPropagator` as well.
...
Add AWS X-Ray id generator, AWS X-Ray propagator, and AWS client instrumentation to your application. The below example is for an ASP.NET Core application.
