...
A collector receiver can populate a connection ip, the ip address in which the signal originated from, in the telemetry signals context for use by subsequent components. 
By default, if the connection ip is available, the Kubernetes attributes processor will use this connection ip to associate pod 
metadata. Additional association rules can also be specified if the connection ip is not available. The Kubernetes attributes processor 
