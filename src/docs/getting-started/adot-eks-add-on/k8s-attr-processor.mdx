---
title: 'Kubernetes Attributes Previousrocessor'
description:
...
with Kubernetes (k8s) metadata. More information in OpenTelemetry resources can be found in the official 
...
After the k8s attributes processor associates a data point with a k8s pod it will automatically attach k8s pod 
metadata as resource attributes. The default list of resource attributes populated can be found 
[here](https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/processor/k8sattributesprocessor/config.go). 
Additionalresource attributes from a pre-defined list can be selected using the configuration. Knowing 
...
processor. Second, we will present a few different places where k8s resource attributes can be used after 
...
before exporting them out of the cluster. To read more about the gateway deployment pattern see the official
OpenTelemetry documentation [here](https://opentelemetry.io/docs/collector/deployment/gateway/). Telemetry 
signals not sent directly to the gateway collector must have k8s resource attributes populated so that the 
k8s attributes processor can perform associations. OpenTelemetry SDKs allows [configuration of resource 
attributes](https://opentelemetry.io/docs/concepts/sdk-configuration/general-sdk-configuration/#otel_resource_attributes) 
through environment variables. The [k8s downward api](https://kubernetes.io/docs/concepts/workloads/pods/downward-api/) 
...
which are then exported to Amazon Managed Prometheus (AMP) and X-Ray respectively.
...
our k8s attributes processor. During this step we also add the indexed_attributes field in the x-ray 
exporter. The indexed_attributes option ensures that the k8s.node.name resource attribute is 
searchable in x-ray. By default, resource attributes are added to trace segment metadata. 
...
After our k8s attributes processor is configured we can then leverage additional processors that are 
available in the ADOT Collector. To drop metrics we will utilize the filter processor and to sample 
out traces we will leverage the group by trace and tail sampling processor. To read more about 
the group by trace and tail sampling processor please visit the Advanced Sampling [documentation page](/docs/getting-started/advanced-sampling). 
...
to show you the different ways k8s resource attributes can be used within a collector pipeline. 
...
Kubernetes resource attributes are useful outside of the collector also. Below are 
...
or aggregating data across a specific set of k8s resources. 
...
See [complex keywords](https://docs.aws.amazon.com/xray/latest/devguide/xray-console-filters.html#console-filters-complex) 
for more information. Use the indexed_attributes configuration option in the X-Ray exporter to ensure 
...
to make k8s resource attributes available to the EMF Exporter. New dimension sets, dashboards, 
