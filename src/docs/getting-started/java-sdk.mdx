---
title: 'Java Getting Started - Application Metrics'
description:
...
[AOT with application metrics sample application](https://github.com/aws-observability/aws-otel-collector-test-framework/blob/master/java-sdk-emitter/src/main/java/com/amazon/aocagent/MetricEmitter.java)
...
1. Create `OtlpGrpcMetricExporter` to send application metrics data to AOC.
```java
...
4. Record Metric measurements and wait for IntervalMetricReader to send the batched Metrics to AOC.
```java
...
Please follow <Link to="/docs/getting-started/aot-cloudwatch-metrics">Getting Started Sending CloudWatch Metrics with AWS OpenTelemetry</Link>
to setup AWS OTel Collector for sending metrics to CloudWatch. Once AOC is installed to collect the metrics data. You should see the following
metrics present on your CloudWatch Console.
