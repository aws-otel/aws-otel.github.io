...
When initializing the OpenTelemetry SDK, register the `AwsXrayRemoteSampler`. Moreover, You can also set the `pollingInterval` and `endpoint` when initializing sampler shown in below code snippet.
