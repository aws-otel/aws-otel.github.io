...
dependencies {
    implementation('io.opentelemetry:opentelemetry-sdk:0.15.0')
}
```
...
##### For Gradle:
```java
implementation 'io.opentelemetry.instrumentation:opentelemetry-otlp-exporter-starter:0.15.0'
```
...
main class, or early in the main method, configure the SDK with the OTLP exporter as follows:

```java lineNumbers=true
OpenTelemetrySdk.builder()
  .addSpanProcessor(BatchSpanProcessor.builder(
    OtlpGrpcSpanExporter.builder()
      .setEndpoint("localhost:11111")
      .build())
    .build())
  .buildAndRegisterGlobal();
```

This initializes the OTLP span exporter to connect to a collector at port `11111` on `localhost`. You can use additional
methods on the `OtlpGrpcSpanExporterBuilder` for additional configuration of the exporter.
...
##### For Gradle:
```java
implementation 'io.opentelemetry.instrumentation:opentelemetry-aws-sdk-2.2:0.15.0'
```
...
```java
implementation 'io.opentelemetry:opentelemetry-sdk-extension-aws:0.15.0'
```
...
And when configuring the SDK, specify the ID generator.
