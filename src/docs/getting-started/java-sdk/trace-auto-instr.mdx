...
to allow the application to fetch sampling configuration. By setting `OTEL_TRACES_SAMPLER_ARG=endpoint=localhost:4000` environment variable you can change the endpoint the sampler talks with when getting sampling configuration from AWS X-Ray Console.
