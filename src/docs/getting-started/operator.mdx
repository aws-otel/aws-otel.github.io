...
description:
    This page is about getting started with the ADOT Operator.
...
Welcome to the AWS Distro for OpenTelemetry Operator (ADOT Operator) getting started guide. This guide covers the ADOT Operator,
...
The ADOT Operator is an implementation of a Kubernetes Operator. A Kubernetes Operator is a method of packaging, deploying and
...
an extension mechanism in Kubernetes. In our case, the CRD that is managed by the ADOT Operator is the Collector.

The ADOT Operator will watch for the Collector and is notified about its presence or modification. When the ADOT Operator receives
...
Once the ADOT Operator deployment is ready, you can deploy the ADOT Collector in your EKS cluster. Managed by the Operator, the Collector
...
end-to-end workflow of ADOT Operator.
