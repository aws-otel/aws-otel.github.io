...
Run `go get go.opentelemetry.io/contrib/detectors/aws/ecs` command to import the ECS resource detector module. The following code snippet demonstrates how to use the ECS resource detector.
```go lineNumbers=true
...
Below example displays how to add attributes to the span. You can convert some or all attributes to annotations via [collector config](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/7bf2266a025425993a233f66c77a0810ab11a78b/exporter/awsxrayexporter#aws-specific-attributes), and otherwise they are metadata by default.
```go lineNumbers=true
var tracer = otel.Tracer("demo")
_, span := tracer.Start(
			context.Background(),
