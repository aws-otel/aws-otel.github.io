---
title: 'Using the AWS Distro for OpenTelemetry Go SDK with X-Ray and Cloudwatch'
description:
...
path: '/docs/getting-started/go-sdk/trace-metric-manual-instr'
---
...
configure the ADOT components to capture traces and metrics with OpenTelemetry Go and AWS X-Ray/Cloudwatch, as well as how to use the AWS Elastic Container Service
...
Note: You’ll also need to have the [ADOT Collector](https://aws-otel.github.io/docs/getting-started/collector) running to export traces to X-Ray and metrics to Cloudwatch.
...
### Sending Traces to AWS X-Ray and Metrics to Cloudwatch
...
OpenTelemetry Go requires an exporter to send traces or metrics to a backend. Exporters allow telemetry data to be transferred either to the AWS Distro for OpenTelemetry Collector (ADOT Collector), or to a remote system or console for further analysis. The ADOT Collector is a separate process that is designed to be a "sink" for telemetry data emitted by many processes, which can then export
...
To add support for metrics we can also create an exporter to send metrics to a backend.  We can initialize the OTLP exporter for metrics by adding the following code after the above setup.
...
pass `otlptracegrpc.WithTLSCredentials()` and `otlpmetricgrpc.WithTLSCredentials()` instead of `otlptracegrpc.WithInsecure()` and `otlpmetric.WithInsecure()` respectively.

#### Creating a Tracer Provider and Meter Provider

In order to generate traces or metrics, OpenTelemetry Go SDK requires a tracer provider or meter provider to be created. A tracer provider can have multiple different span processors, which are components that give the ability to modify and export span data after it has been created.
...
To create a new meter provider with a peridiodic reader, add the following lines after the above snippet.
```go lineNumbers=true
...
In addition to setting a global tracer provider and meter provider, we will also configure the context propagation option. Context propagation refers to sharing data across multiple processes or services. Propagator structs are configured inside Tracer structs to support context propagation across process boundaries. A context will often have information
