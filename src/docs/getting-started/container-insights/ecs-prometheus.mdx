...
Follow these steps to run the collector and Prometheus workload on ECS. You can skip some of them if you already have clusters or tasks running.
...
- Deploy [collector](#deploy-collector) as a replica service with 1 replica
...
For the collector to scrape metrics, connectivity between ECS tasks using private ip is required.
...
Collector needs to run as single instance. We use the replica service because it handles task failure.
...
- `logConfiguration` is for log from collector itself. Log groups for metrics/log collected by collector is configured using `AOT_CONFIG_CONTENT`.
