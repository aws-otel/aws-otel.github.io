...
### ADOT Operator and Collector Lifecycle Management

The ADOT Operator and Collector both are continuously updated and new versions are being released. Check for an update of the ADOT Operator using the command
```
aws eks describe-addon --addon-name adot --cluster-name yourcluster-name
```
Refer [here](https://docs.aws.amazon.com/eks/latest/userguide/managing-kube-proxy.html) for more information. If a new version of the ADOT Operator is found, update the ADOT EKS Add-On in your cluster with the command
```
aws eks update-addon --addon-name adot --cluster-name your-cluster-name
```
When a new version of the ADOT Collector is released, reapply the Collector configurations using an updated ADOT Collector image.
...
For more information about Collector configuration, as well as Prometheus receiver configuration and AMP, CloudWatch, and X-Ray exporter configurations, refer to the documentation below:
...
* [AMP Exporter configuration](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/awsprometheusremotewriteexporter#readme)
* [CloudWatch Exporter configuration](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/awsemfexporter#readme)
* [X-Ray Exporter configuration](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/awsxrayexporter#readme)
...
The Collector configuration below is set up to receive both OTLP metrics and Prometheus and export to AMP. 
...
Run the following command to check if AMP received the OTLP metrics data `actualQueueSize`.
```
...
Run the following command to check if AMP received the Prometheus metrics data `scrape_duration_seconds`.
```
awscurl --service="aps" --region="YOUR-AMP-REGION" \
"https://aps-workspaces.YOUR-AMP-REGION.amazonaws.com/workspaces/YOUR-AMP-WORKSPACE-ID/api/v1/query?query=scrape_duration_seconds"
```
Your output should look something like this:
```
...
Make sure to replace the `YOUR_AWS_REGION` fields and `YOUR_AMP_WORKSPACE_ID` fields for both queries.
...
status: {}
```
One important note about `traffic-generator.yaml` is that it makes http calls to `sample-app:4567`. `sample-app` is the name of the Kubernetes service that allows our traffic generator the ability to interact with our sample application, on port `4567`. `sample-app` resolves to the IP address of the `sample-app` pod.
...
status: {}
```
There are a few important callouts within `sample-app.yaml`. First, under our `Service` resource, we see `port:4567`, which our traffic generator is configured to make http calls to. Next, in our `Deployment` resource, the environment variables section, `env:`, has a few things to note there. First, you want to change your `AWS_REGION` value, `<YOUR_AWS_REGION>`, to your own region. Next, notice that the `LISTEN_ADDRESS` is listening on `0.0.0.0:4567` for http calls from our traffic generator. Most importantly, we see that `OTEL_EXPORTER_OTLP_ENDPOINT` has a value of `http://my-collector-collector:4317`. `my-collector-collector` is the name of the Kubernetes service that allows our sample application to interact with our ADOT Collector, on port `4317`. In our Collector configuration, we will see that the Collector receives metrics/traces from an `endpoint: 0.0.0.0:4317`. 
...
IMPORTANT: Before looking through the troubleshooting section, make sure you meet all the prerequisites laid out in the guide above, and do not miss any steps during setup. 

### `yaml` File Parsing Error:
```
Error: cannot load configuration's parser: error loading config file "/conf/collector.yaml": unable to read the file /conf/collector.yaml:
```
* If you are getting this error, try downloading the configuration files with the links provided

### Errors when Creating the Add-On:
```
            "issues": [
                {
                    "code": "K8sResourceNotFound",
                    "message": "cert-manager is not installed on this cluster. During preview, you are required to have previously installed cert-manager."
                }
            ]
```
* Make sure your cert-manager is a version that is supported (<1.6.0)

```
            "issues": [
                {
                    "code": "AccessDenied",
                    "message": "roles.rbac.authorization.k8s.io \"opentelemetry-operator-leader-election-role\" is forbidden: User \"eks:addon-manager\" cannot patch resource \"roles\" in API group \"rbac.authorization.k8s.io\" in the namespace \"opentelemetry-operator-system\""
                }
```
* This happens when you delete the Add-On and don’t reapply permissions before reinstalling it again
