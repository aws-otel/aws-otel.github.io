...
Go to your IAM Management Console to find the **amp-iamproxy-ingest-role** that was created in the prerequisites. Go to trust relationships and click **Edit trust relationship.**
...
To start a default instance of the Collector, we have provided an example template configuration. You will have to change the `<YOUR_ENDPOINT>` 
and `<YOUR_REGION>` to correspond with your own AMP workspace.  You’ll also need to change `<YOUR_ACCOUNT_ID>` to your AWS account ID. The changes 
should be with respect to the AWS Prometheus Remote Write Exporter’s configuration. 
