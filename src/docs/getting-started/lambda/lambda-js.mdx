...
<WarningBox
  warningText="This documentation describes the legacy approach to using ADOT with Lambda. For the latest recommended approach, please refer to the updated Python Lambda documentation."
  buttonText="View Updated Docs →"
  hyperlink="/docs/getting-started/lambda"
/>
...
|---------------------|-------------------------|----------|
| ap-northeast-1<br/>ap-northeast-2<br/>ap-south-1<br/>ap-southeast-1<br/>ap-southeast-2<br/>ca-central-1<br/>eu-central-1<br/>eu-north-1<br/>eu-west-1<br/>eu-west-2<br/>eu-west-3<br/>sa-east-1<br/>us-east-1<br/>us-east-2<br/>us-west-1<br/>us-west-2 | arn:aws:lambda:<region\>:901920570463:layer:aws-otel-nodejs-<architecture\>-ver-1-30-2:6 |Contains [OpenTelemetry for JavaScript v1.30.0](https://github.com/open-telemetry/opentelemetry-js/releases/tag/v1.30.0) with [Lambda instrumentation v0.50.3](https://github.com/open-telemetry/opentelemetry-js-contrib/releases/tag/instrumentation-aws-lambda-v0.50.3) <br/><br/> Contains ADOT Collector v0.49.0 |
...
5. Specify the `OTEL_NODE_ENABLED_INSTRUMENTATIONS` environment variable in the lambda to configure the instrumentations needed. By default the layer is set to `OTEL_NODE_ENABLED_INSTRUMENTATIONS="aws-sdk,aws-lambda,http"` and are loaded automatically by default.
    - The environment variables should be set to a comma-separated list of the instrumentation package names without the @opentelemetry/instrumentation- prefix
6. [Enable active tracing](https://docs.aws.amazon.com/lambda/latest/dg/services-xray.html) for your AWS Lambda function.
