...
By using OpenTelemetry SDK, we can add the customized attributes for each trace subsegment. By default, these OTel
attributes will be converted into `metadta` attributes in X-Ray raw data. If we need to convert all of OTel attributes or the selected attributes
