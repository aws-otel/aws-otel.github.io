...
| http.url, http.scheme, http.host, http.target, http.server_name, net.host.port, host.name, net.host.name, net.peer.name, net.peer.port, net.peer.ip | http.url               | <ul><li> if span.kind = "Server" then construct server URL. Otherwise contruct a client URL.  </li></ul>                                                                                                         |
| message.type                                                                                                                                        | http.content_length    | <ul><li> If message.type attribute key exists and value = "RECEIVED" then http.content_length is set to messaging.message_payload_size_bytes if it is set; otherwise http.content_length is set to 0. </li></ul> |
