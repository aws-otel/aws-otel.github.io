...
To use [AWS Systems Manager Distributor](https://docs.aws.amazon.com/systems-manager/latest/userguide/distributor.html) to install AWSDistroOTel-Collector package, the EC2 instance need to meet follwoing requiements.
...
2. Copy AWS Distro for OpenTelemetry Collector YAML configuration file encoded as Base64 to a new SSM parameter in [SSM Parameter Store](https://us-west-2.console.aws.amazon.com/systems-manager/parameters/).
...
5. Continue to select the EC2 instance in Targets and click Run button to start installation.
