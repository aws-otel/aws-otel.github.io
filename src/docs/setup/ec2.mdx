...
1. Download the CloudFormation template file `aoc-ec2-deployment-cfn.template` for installing AWS OTel Collector on  Amazon EC2.
```
curl -O https://raw.githubusercontent.com/aws-observability/aws-otel-collector/master/deployment-template/ec2/aoc-ec2-deployment-cfn.template?token=ABIVU6M6N367QIKL7XZU2FK7RHBNO
```
...
aws cloudformation create-stack --stack-name AOCEC2-Test \
    --template-body file://<*template_path*>/aoc-ec2-deployment-cfn.template \
...
```
5. Check if  AWS Observability Collector is set up and running on the new EC2 instance. If it is, you’ll see the following status.
```
