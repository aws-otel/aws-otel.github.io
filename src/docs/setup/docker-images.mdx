---
title: 'Run AWS Distro for OpenTelemetry Demo Application in Docker'
description:
    This example will introduce how to install AOC to collect metrics/traces data from a sample application running in
    the Docker container. The sample application will generate Open Telemetry Protocol (OTLP) format based metrics and
    traces data and send to AWS CloudWatch and X-Ray consoles. Please follow the steps below to try AWS Observability Collector Beta.
...
This example will introduce how to install AOC to collect metrics/traces data from a sample application running in the
...
3. Once AOC and Sample application docker images are up and running. We can check the monitoring data in CloudWatch and X-Ray consoles.
...
6. To stop this demo with AOC, please run the following command.
```
