...
To view your current AppDynamics build version in the Controller UI, go to ![Settings](images/geariconblack-white.png) > **About AppDynamics**.
...
Users must have a role with the **View and Configure Licenses** permission for license management activity. To view your current AppDynamics edition in the Controller UI, go to ![Settings]( images/geariconblack-white.png) > License.
...
- `service.name`: your AppDynamics tier name. Set corresponding `service.name` trace resource attribute for every service being monitored either via SDK.  set the resource attributes.
- `service.namespace`: your AppDynamics application name. Set corresponding `service.namespace` trace resource attribute for every service being monitored either via SDK.
  - You can set these `service.name` and `service.namespace` attributes:
    ...
    2. Inside your application code: please consult the appropriate language documentation on how to
