---
title: 'Getting Started with Dynatrace metrics exporter in AWS OpenTelemetry Collector'
description:
...
path: '/docs/components/dynatrace-exporter'
---
...
The Dynatrace Open Source Engineering team wrote an exporter for metrics in the OpenTelemetry Collector. This exporter allows
metrics collected by the OpenTelemetry Collector to be stored, viewed, and analyzed in Dynatrace.
...
## Enable Dynatrace Metrics Exporter

To enable the Dynatrace metrics exporter, write the name under exporter section in the OT config file `(local/config.yaml)` and add the `dynatrace` exporter to your metrics pipeline.
...
Customers can configure the HTTP client, TLS, queueing, retry, and timeout settings.
